...
- The workflow hotkey is a transient start/stop toggle for continuous trading; a second press stops local input only and never cancels or forgets a server order. Authorization is never persisted, and reload, area/target/permission change, hard controller failure, or ambiguity requires another press.
- While authorized, a completed or safely stopped workflow starts a fresh scan only after the durable `Stashed` transition leaves no unresolved order, reservation, uncollected settlement, or pending batch. A planner-complete no-route scan retries without consuming the transient budget. A controller-recommended transient market capture, stable-book, unavailable-quote, staging-quote, or final live-quote failure before any placement click may schedule up to 10 fresh coherent reprobes for that leg, using the jittered 10s cooldown (2-90s configurable, -2..+2s). The next transient failure revokes authorization. Focus, cursor, permission, UI, area, target, calibration, persistence, canonical-state, and post-click failures always stop immediately.
- Every transient preparation retry writes its original reason and `retry N/10` to runtime diagnostics before clearing the displayed failure. Exhaustion writes `WorkflowPreparationRetryExhausted` and `ContinuousAuthorizationRevoked`. Every other continuous-authorization revocation writes `ContinuousAuthorizationRevoked` with its reason, and every loop-decision change plus a 30s heartbeat records the blocking component and next scan. A silently idle authorized loop is a defect. A placement latch that no controller has owned for 5s holds no armed input and is released; nothing else auto-recovers.
